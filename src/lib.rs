//! # Bytestat
//!
//! Bytestat is a crate to measure randomness of data. Data is processed one byte at a time, 
...
/// Analyze one byte, repeat as needed.
///
/// # Examples
///
/// ```
...
/// ```
pub fn get_score_non_zero(&self) -> f64 {
    ...
    dist_not_zero as f64 / 256 as f64
  }
...
/// ```
pub fn get_score_unique(&self) -> f64 {
...
/// ```
pub fn get_score_amplitude(&self) -> f64 {
    ...
    (dist_amp_max - dist_amp_variation) as f64 / dist_amp_max as f64
  }

/// Generate the score based on the amplitude of the continuity of significant interval measurements.
/// The score is between 0.0 and 1.0. Any score lower than 0.99 should be considered problematic.
/// 
...
/// stats.get_score_amplitude()
/// ```
pub fn get_score_interval_continuity(&self) -> f64 {
...
/// ```
pub fn get_score_interval_amplitude(&self) -> f64 {
    let mut interval_min = std::u16::MAX;
    let mut interval_max = std::u16::MIN;
    ...
    interval_max as f64 / 512 as f64
  }

/// Generate the general score based on the 5 individual tests. 
/// Score between 0.0 and 100.0. 99.0 or lower is very problematic.
///
/// # Examples
///
/// ```
...
pub fn get_score(&self) -> f64 {
    let mut score;
    score = self.get_score_non_zero() * 20f64;
    score += self.get_score_unique() * 20f64;
    score += self.get_score_amplitude() * 20f64;
    score += self.get_score_interval_continuity() * 20f64;
    score += self.get_score_interval_amplitude() * 20f64;
    score
  }
}
